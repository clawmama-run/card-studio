...
Render with the CLI in this skill/repo directory (see SKILL.md setup):

```bash
node <skill-dir>/dist/cli.js --data spec.json --out card.png       # single
node <skill-dir>/dist/cli.js --data specs.json --out-dir out/post  # array = batch
```
...
Each template declares compatible themes (`node <skill-dir>/dist/cli.js list`);
...
```ts
import { renderCard } from '<skill-dir>/dist/index.js';
